...
function getBlocksForYear(year, data, fullYear) {
  const now = new Date();
  const firstDate = fullYear ? subYears(now, 1) : parseISO("".concat(year, "-01-01"));
  const lastDate = fullYear ? now : parseISO("".concat(year, "-12-31"));
...
const GitHubCalendar = ({
  ...
  function renderTitle() {
    const style = {
      borderBottom: "2px solid ".concat(getTheme().grade0),
  ...
  function renderMonthLabels(monthLabels) {
    ...
    return monthLabels.map(month => React__default.createElement("text", {
  ...
  function renderBlocks(blocks) {
    ...
    return blocks.map(week => week.map((day, y) => React__default.createElement("rect", {
      x: "0",
      ...
      key: day.date
    }))).map((week, x) => React__default.createElement("g", {
      key: x,
